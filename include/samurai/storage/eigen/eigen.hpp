...
namespace samurai
{
    ...
    template <class value_t, bool SOA>
    auto view(eigen_container<value_t, 1, SOA>& container, const contiguous_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range.start, range.end - 1));
    }

    template <class value_t, bool SOA>
    auto view(eigen_container<value_t, 1, SOA>& container, const strided_range_t<Eigen::Index>& range)
    {
    ...
    auto view(eigen_container<value_t, size, true>& container, const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::placeholders::all, Eigen::seq(range.start, range.end - 1, range.step));
    }

    template <class value_t, std::size_t size, typename = std::enable_if_t<(size > 1)>>
    auto view(eigen_container<value_t, size, false>& container, const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range.start, range.end - 1, range.step), Eigen::placeholders::all);
    }

    template <class value_t, std::size_t size>
    auto view(eigen_container<value_t, size, true>& container,
              const strided_range_t<Eigen::Index>& range_item,
              const strided_range_t<Eigen::Index>& range)
    {
    ...
    template <class value_t, std::size_t size>
    auto view(eigen_container<value_t, size, false>& container,
              const strided_range_t<Eigen::Index>& range_item,
              const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range.start, range.end - 1, range.step),
    ...
    template <class value_t, std::size_t size>
    auto view(eigen_container<value_t, size, true>& container, std::size_t item, const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(item, Eigen::seq(range.start, range.end - 1, range.step));
    }

    template <class value_t, std::size_t size>
    auto view(eigen_container<value_t, size, false>& container, std::size_t item, const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range.start, range.end - 1, range.step), item);
    }

    template <class value_t, bool SOA>
    auto view(const eigen_container<value_t, 1, SOA>& container, const contiguous_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range.start, range.end - 1));
    }

    template <class value_t, bool SOA>
    auto view(const eigen_container<value_t, 1, SOA>& container, const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range.start, range.end - 1, range.step));
    }

    template <class value_t, std::size_t size, typename = std::enable_if_t<(size > 1)>>
    auto view(const eigen_container<value_t, size, true>& container, const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::placeholders::all, Eigen::seq(range.start, range.end - 1, range.step));
    }

    template <class value_t, std::size_t size, typename = std::enable_if_t<(size > 1)>>
    auto view(const eigen_container<value_t, size, false>& container, const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range.start, range.end - 1, range.step), Eigen::placeholders::all);
    }

    template <class value_t, std::size_t size>
    auto view(const eigen_container<value_t, size, true>& container,
              const strided_range_t<Eigen::Index>& range_item,
              const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range_item.start, range_item.end - 1, range_item.step),
    ...
    template <class value_t, std::size_t size>
    auto view(const eigen_container<value_t, size, false>& container,
              const strided_range_t<Eigen::Index>& range_item,
              const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range.start, range.end - 1, range.step),
    ...
    template <class value_t, std::size_t size>
    auto view(const eigen_container<value_t, size, true>& container, std::size_t item, const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(item, Eigen::seq(range.start, range.end - 1, range.step));
    }

    template <class value_t, std::size_t size>
    auto view(const eigen_container<value_t, size, false>& container, std::size_t item, const strided_range_t<Eigen::Index>& range)
    {
        return container.data()(Eigen::seq(range.start, range.end - 1, range.step), item);
    }
